...
export function DeficitPanel({ content, crossingExplanation, advanceLabel }: Props) {
  ...
  return (
    <section className="rounded-lg bg-slate-800/95 p-5 text-slate-100 shadow-lg">
      <h2 className="mb-2 text-xl font-semibold">{scenario.label}</h2>

      {predictedLabel && (
        <p className="mb-3 rounded-md bg-slate-700/60 p-3 text-base">
          {wasRight
            ? 'You called it.'
      ...
      <p className="mb-3 text-base leading-relaxed text-slate-300">{scenario.dayToDay}</p>

      <p className="mb-3 rounded-md border-l-2 border-amber-400 bg-slate-900/60 p-3 text-base text-slate-300">
      ...
      {scenario.surprise && <p className="mb-4 text-base leading-relaxed text-amber-200">{scenario.surprise}</p>}
