...
import type { MotorExamProfile } from '@content/schema';
import { resolveTrial } from '@levels/parkinsons/simulation/resolveTrial';
...
function ProfileCard({ title, profile, tone }: { title: string; profile: MotorExamProfile; tone: 'intact' | 'damaged' }) {
  ...
  return (
    <div className={`flex flex-col gap-2 rounded-md border ${border} bg-slate-900/60 p-3`}>
      ...
      <ul className="flex flex-col gap-1">
        <StatusRow label="Resting tremor" ok={!profile.restingTremor} valueLabel={profile.restingTremor ? 'present' : 'absent'} />
        <StatusRow label="Rigidity" ok={profile.rigidity === 'normal'} valueLabel={profile.rigidity} />
        <StatusRow label="Movement speed" ok={profile.movementSpeed === 'normal'} valueLabel={profile.movementSpeed} />
        <StatusRow label="Posture" ok={profile.posturalStability === 'stable'} valueLabel={profile.posturalStability} />
      </ul>
...
export function MotorExamDashboard() {
  const { state, dispatch } = useSimulation();
  if ((state.phase !== 'revealed' && state.phase !== 'rehab') || !state.site) return null;

  const result = resolveTrial(state.site, parkinsons);
  ...
  return (
    <section className="rounded-lg bg-slate-800/95 p-5 text-slate-100 shadow-lg">
      ...
      <div className="mt-3 grid grid-cols-2 gap-3">
        <ProfileCard title="Normal dopamine" profile={result.trial.intact} tone="intact" />
        <ProfileCard title="Dopamine depleted" profile={result.trial.damaged} tone="damaged" />
      </div>
