...
/** No BodyParts3D part exists for the substantia nigra; approximate midbrain position. */
const SUBSTANTIA_NIGRA_POSITION: [number, number, number] = [0, -20, -9];

export function ParkinsonsBrainMesh() {
  ...
  const materials = useMemo(
    () => ({
      ...
      striatum: new THREE.MeshStandardMaterial({
        color: '#fb923c', emissive: '#c2410c', emissiveIntensity: 0.7, roughness: 0.45,
      }),
    }),
    []
  );
  ...
  useLayoutEffect(() => {
    ...
    scene.traverse((child) => {
      ...
      if (child.name.startsWith('Amygdala') || child.name.startsWith('Hippocampus')) {
  ...
  return (
    <>
      ...
      {state.phase !== 'overview' && (
        <mesh position={SUBSTANTIA_NIGRA_POSITION} renderOrder={1}>
          <sphereGeometry args={[5, 24, 24]} />
          <meshStandardMaterial color="#1e293b" emissive="#0f172a" emissiveIntensity={1.4} />
        </mesh>
      )}
    </>
  );
