import type { MotorExamContent, MotorExamTrial } from '@content/schema';

export interface TrialResult {
  trial: MotorExamTrial;
  correctChoiceId: string;
}

export function trialIds(content: MotorExamContent): string[] {
  return content.trials.map((t) => t.id);
}

export function resolveTrial(trialId: string, content: MotorExamContent): TrialResult {
