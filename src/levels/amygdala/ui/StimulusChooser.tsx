...
export function StimulusChooser() {
  ...
  return (
    <nav aria-label="Choose a situation" className="rounded-lg bg-slate-800/95 p-5 text-slate-100 shadow-lg">
      <h2 className="mb-1 text-lg font-semibold">Put them in a situation</h2>
      <p className="mb-4 text-sm text-slate-300">Both amygdalae are damaged. What happens when…</p>
        ...
        {amygdala.stimuli.map((s) => (
          <li key={s.id}>
            ...
            >
              ...
              <span className="mt-0.5 block text-xs text-slate-300">{s.description}</span>
            </button>
          </li>
        ))}
      </ul>
