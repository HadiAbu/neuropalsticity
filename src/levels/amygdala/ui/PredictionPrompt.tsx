...
export function PredictionPrompt() {
  ...
  const stimulus = amygdala.stimuli.find((s) => s.id === state.site);
  if (!stimulus) return null;

  return (
    <section className="rounded-lg bg-slate-800/95 p-5 text-slate-100 shadow-lg">
      <p className="text-sm uppercase tracking-wide text-amber-300">{stimulus.label}</p>
      <h2 className="mb-1 mt-1 text-xl font-semibold">Before you look — what do they do?</h2>
      <p className="mb-4 text-base text-slate-300">{stimulus.description}</p>
      <ul className="flex flex-col gap-2">
        {stimulus.choices.map((choice) => (
          <li key={choice.id}>
