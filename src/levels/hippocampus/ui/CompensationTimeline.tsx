...
export function CompensationTimeline() {
  ...
  const { timeline, mechanism, caveat } = hippocampus.plasticity;
  ...
  return (
    <section className="rounded-lg bg-slate-800/95 p-5 text-slate-100 shadow-lg">
      <h2 className="mb-2 text-lg font-semibold">The world starts remembering for them</h2>
      <p className="mb-4 text-sm leading-relaxed text-slate-300">{mechanism}</p>

      <label htmlFor="compensation-week" className="mb-1 block text-sm font-medium">
        {when} — {Math.round(current.recoveryFraction * 100)}% of external strategies adopted
      </label>
      <input
