...
export function PredictionPrompt() {
  ...
  const trial = hippocampus.trials.find((t) => t.id === state.site);
  if (!trial) return null;

  return (
    <section className="rounded-lg bg-slate-800/95 p-5 text-slate-100 shadow-lg">
      <p className="text-xs uppercase tracking-wide text-amber-300">{trial.label}</p>
      <h2 className="mb-1 mt-1 text-lg font-semibold">Before you look — what happens?</h2>
      <p className="mb-4 text-sm text-slate-300">{trial.description}</p>
