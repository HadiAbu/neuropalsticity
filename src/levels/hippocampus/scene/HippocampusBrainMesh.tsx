...
const isHippocampus = (name: string) => name.startsWith('Hippocampus');

export function HippocampusBrainMesh() {
  ...
  const materials = useMemo(
    () => ({
      ...
      hippocampus: new THREE.MeshStandardMaterial({
        color: '#c084fc', emissive: '#9333ea', emissiveIntensity: 0.9, roughness: 0.4,
      }),
    }),
    []
  );
  ...
  useLayoutEffect(() => {
    ...
    scene.traverse((child) => {
      ...
      if (isHippocampus(child.name)) {
        child.material = materials.hippocampus;
        child.renderOrder = 1;
        return;
      }
