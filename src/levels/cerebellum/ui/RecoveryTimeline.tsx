...
export function RecoveryTimeline() {
  ...
  const { timeline, mechanism, caveat } = cerebellum.plasticity;
  ...
  return (
    <section className="rounded-lg bg-slate-800/95 p-5 text-slate-100 shadow-lg">
      <h2 className="mb-2 text-xl font-semibold">Retuning the loop</h2>
      <p className="mb-4 text-base leading-relaxed text-slate-300">{mechanism}</p>

      <label htmlFor="recovery-week" className="mb-1 block text-base font-medium">
        {when} — {Math.round(current.recoveryFraction * 100)}% of balance and coordination regained
      </label>
      <input
        id="recovery-week"
        type="range"
