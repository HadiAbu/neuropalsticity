...
function LevelRow({ level, hovered, onHover, onEnter }: RowProps) {
    ...
    return (
      ...
      >
        <span>
          ...
          <span className="block text-sm text-slate-300">{level.name}</span>
        </span>
  ...
  return (
    ...
    >
      ...
      <span className="shrink-0 rounded bg-amber-500 px-2 py-0.5 text-xs font-medium text-slate-900">{hubCopy.enter}</span>
    </button>
  );
}
  ...
  return (
    <main className="relative h-screen w-screen overflow-hidden bg-slate-900 text-slate-100">
      ...
      <header className="pointer-events-none absolute left-6 top-6 max-w-md">
        ...
        <p className="mt-2 text-lg text-slate-300">{hubCopy.tagline}</p>
        <p className="mt-3 hidden text-xs text-slate-400 md:block">{hubCopy.hint}</p>
      </header>
        ...
        {THEME_ORDER.map((theme) => {
          ...
          return (
            <section key={theme} aria-labelledby={`theme-${theme}`} className="mb-5">
              <h2 id={`theme-${theme}`} className="mb-2 text-xs font-medium uppercase tracking-wide text-slate-400">
                {themeTitles[theme]}
              </h2>
